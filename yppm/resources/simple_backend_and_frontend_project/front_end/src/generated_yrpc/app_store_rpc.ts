import * as app_store_objects from './app_store_objects'

export class Client_app_store {
  /**
   ...
   */
    ...
    async _get_reponse_or_error_by_url_path_and_input(sub_url: string, input_dict: { [key: string]: any }): Promise<any> {
        let the_url = `${this._service_url}/app_store/${sub_url}/`
        try {
    ...
    async add_app(item: app_store_objects.Add_App_Request, ignore_error?: boolean): Promise<app_store_objects.Add_App_Response | null> {
        let result = await this._get_reponse_or_error_by_url_path_and_input("add_app", item.to_dict())
        if (Object.keys(result).includes(this._special_error_key)) {
            ...
            return new app_store_objects.Add_App_Response().from_dict(result)
        }
    }

    async search_app(item: app_store_objects.Search_App_Request, ignore_error?: boolean): Promise<app_store_objects.Search_App_Response | null> {
        let result = await this._get_reponse_or_error_by_url_path_and_input("search_app", item.to_dict())
        if (Object.keys(result).includes(this._special_error_key)) {
            if ((ignore_error == null) || ((ignore_error != null) && (!ignore_error))) {
                this._error_handle_function(result[this._special_error_key])
            }
            ...
            return new app_store_objects.Search_App_Response().from_dict(result)
        }
    }

    async get_app_detail(item: app_store_objects.Get_App_Detail_Request, ignore_error?: boolean): Promise<app_store_objects.Get_App_Detail_Response | null> {
        let result = await this._get_reponse_or_error_by_url_path_and_input("get_app_detail", item.to_dict())
        if (Object.keys(result).includes(this._special_error_key)) {
            if ((ignore_error == null) || ((ignore_error != null) && (!ignore_error))) {
                this._error_handle_function(result[this._special_error_key])
            }
            ...
            return new app_store_objects.Get_App_Detail_Response().from_dict(result)
        }
    }

    async export_data(item: app_store_objects.Export_Data_Request, ignore_error?: boolean): Promise<app_store_objects.Export_Data_Response | null> {
        let result = await this._get_reponse_or_error_by_url_path_and_input("export_data", item.to_dict())
        if (Object.keys(result).includes(this._special_error_key)) {
            if ((ignore_error == null) || ((ignore_error != null) && (!ignore_error))) {
                this._error_handle_function(result[this._special_error_key])
            }
            ...
            return new app_store_objects.Export_Data_Response().from_dict(result)
        }
    }
}

export default Client_app_store